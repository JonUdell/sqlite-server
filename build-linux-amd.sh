...
if [ ! -d "$SQLITE_INSTALL_DIR" ]; then
  CFLAGS="-DSQLITE_ENABLE_LOAD_EXTENSION=1 -DSQLITE_ALLOW_LOAD_EXTENSION=1" ./configure --prefix=$SQLITE_INSTALL_DI
R --disable-shared --enable-static
...
if [ ! -f steampipe_sqlite_github.so ]; then
  ...
  EXTENSION_URL="https://github.com/turbot/steampipe-plugin-github/releases/download/v1.2.0/steampipe_sqlite_github
.${OS_TYPE}_${ARCH}.tar.gz"
